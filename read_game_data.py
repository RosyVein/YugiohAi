...
def read_data():
    ...
    if (TrainData):
        ...
        if os.path.exists('./data/action') and len(x_train) > 0:
            ...
            clf = pickle.load(file)
            if len(x_train[
                       0]) == clf.n_features_in_ and clf.classes_.all() in y_train and clf.classes_.__len__() >= y_train.__len__():
                clf.partial_fit(x_train, y_train)
            else:
                clf = None
        if clf == None:
            clf = MLPClassifier(activation='relu', solver='adam', max_iter=1000,
                                hidden_layer_sizes=(input_length + output_length, input_length + output_length)).fit(
                x_train, y_train)
            print("New Network")
